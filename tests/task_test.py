...
def test_task_execution():
    ...
    if task_start_time < datetime.strptime(current_task_time,"%H:%M:%S").time() and task_end_time > datetime.strptime(current_task_time,"%H:%M:%S").time():
...
def test_task_creation_weekdays():
    ...
    task = timezone_based_ask_execution.TaskParser.parse(task_input)
    current_task_time,weekday = timezone_based_ask_execution.TaskCreation(task).create_task_with_weekdays()  
    current_time = datetime.now().strftime("%H:%M:%S")
...
def test_task_runner_weekdays_False():
    task_input = "CALL U2 India 13:00:00 14:30:00 Friday,Tuesday"    
    result = timezone_based_ask_execution.TaskController().task_runner_with_weekdays(task_input)
    print(result)    
